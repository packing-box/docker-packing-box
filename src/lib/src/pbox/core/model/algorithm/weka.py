        ...
        def _subwrapper(self, *args, **kwargs):
            ...
            if y is not None:
                ...
                d = [[_fmt_num(v) for v in row[1].tolist()] + [_fmt_nom(tgt)] for row, tgt in zip(X.iterrows(), y)] \
                    if isinstance(X, pd.DataFrame) else [[_fmt_num(v) for v in row] + [_fmt_nom(tgt)] \
                                                         for row, tgt in zip(X, y)]
            else:
                d = [[_fmt_num(v) for v in row[1].tolist()] + ["?"] for row in X.iterrows()] \
                    if isinstance(X, pd.DataFrame) else [[_fmt_num(v) for v in row] + ["?"] for row in X]
